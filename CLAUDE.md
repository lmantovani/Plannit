...
## Módulo RH / Colaboradores (adicionado pelo parceiro) 🆕
Identificado no banco de produção — módulos novos que expandem o schema além do que
está documentado em detalhe neste arquivo. Precisa ser mapeado em detalhe na próxima
atualização deste CLAUDE.md pelo colaborador responsável.

Tabelas confirmadas no banco (via Alembic autogenerate + Supabase/pgAdmin):
- `departamentos`
- `cargos`
- `colaboradores` (indexado por `cpf`)
- `historico_cargo_colaboradores`
- `historico_salarial_colaboradores`
- `documentos_colaboradores`
- `metas_visitas_consultor`

Também expandiu o módulo Arquitetos com:
- `concorrentes_arquitetos`
- `historico_dono_arquitetos`
- `interacoes_arquitetos`
- `decisores_arquitetos` (nome no banco é plural — diferente do `DecisorArquiteto` singular
  usado nos exemplos deste arquivo; confirmar nome real da classe/model)

⚠️ Ação pendente: documentar aqui os endpoints, regras de negócio e telas desses módulos
assim que confirmado com o parceiro, para manter este arquivo como fonte única de verdade.
...
## Alembic — regra obrigatória a partir de agora
- **seed.py** originalmente criava tabelas via `Base.metadata.create_all()` — isso NÃO deve
  mais ser a fonte de verdade do schema. A partir da primeira migration (94d29e691390),
...
- **Se o banco já tiver as tabelas criadas por `create_all()` antes do Alembic existir**
  (nosso caso — banco de produção já tinha tudo, tanto do módulo Projetos/Arquitetos
  quanto do módulo de Colaboradores/RH do parceiro): rodar UMA VEZ
